...
import { useUpdateLotMutation, useAuctionDetails } from "../hooks/useAuctionQueries";
...
export const EditLotView: React.FC = () => {
  ...
  const updateLotMut = useUpdateLotMutation({ auctionId: auctionId || "" });
  ...
  if (!hasRole("SELLER")) {
    return (
      <div className="bg-slate-900/40 border border-slate-800 rounded-2xl p-8 max-w-xl mx-auto text-center font-mono text-xs space-y-4">
        ...
        <p className="text-slate-400">Only SELLER credentials can modify lots.</p>
        <button
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-24 font-mono text-xs text-slate-400">
        ...
        <span>RETRIEVING LOT SPECIFICATIONS...</span>
      </div>
    );
  }
  ...
  if (isError || !auction || (auction.state !== AuctionState.DRAFT && auction.state !== AuctionState.REJECTED)) {
    return (
      <div className="bg-rose-500/10 border border-rose-500/20 rounded-2xl p-6 text-center max-w-xl mx-auto text-rose-400 font-mono text-xs space-y-3">
        ...
        <p className="text-slate-400">Campaign must be in DRAFT mode to modify lots.</p>
        <button
          onClick={() => navigate(`/auctions/${auctionId}/lots`)}
  ...
  const onSubmit = async (data: LotSchemaType) => {
    ...
    setFormError(null);
    try {
      await updateLotMut.mutateAsync({ lotId, request: data });
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto font-mono text-xs">
      <div className="flex items-center gap-3">
        ...
        <div>
          <span className="text-[10px] text-indigo-400 uppercase tracking-widest font-bold">Lot Modification</span>
          <h2 className="text-lg font-bold text-white">UPDATE LOT SPECIFICATIONS</h2>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex justify-end gap-3 pt-6 border-t border-slate-800/40">
          <button
            type="button"
            disabled={updateLotMut.isPending}
            onClick={() => navigate(`/auctions/${auctionId}/lots`)}
