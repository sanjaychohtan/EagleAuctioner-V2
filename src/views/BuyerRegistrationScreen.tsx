...
export const BuyerRegistrationScreen: React.FC = () => {
  ...
  // Sub-steps to reduce cognitive overload by separating Bank and Identity details
  const [businessSubStep, setBusinessSubStep] = useState(0);

  // Simulated document scanning and upload acknowledgment state
  ...
  const methods = useForm<BidderRegistrationSchemaType>({
    ...
    defaultValues: {
      ...
      userRole: "ROLE_BUYER",
      panNumber: "",
      rawAadhaar: "", // Removed hardcoded Aadhaar prefill
      organizationName: "",
      registrationNumber: "",
      gstin: "",
      registeredAddress: "",
  ...
  // Mock File Upload Handler with client-side validations and async acknowledgment simulation
  const handleFileUpload = (docType: string, file: File) => {
    // 1. Client-side validation: Max file size 10MB
    const MAX_SIZE = 10 * 1024 * 1024; // 10MB
    if (file.size > MAX_SIZE) {
      showNotification(`File size exceeds 10MB limit. Your file is ${(file.size / (1024 * 1024)).toFixed(2)}MB.`, "error");
      return;
    }
    ...
    if (!ALLOWED_MIME_TYPES.includes(file.type)) {
      showNotification("Invalid file type. Only PDF, JPG, JPEG, and PNG are allowed.", "error");
      return;
    }

    // Set processing/uploading status for this specific document
    setProcessingDocs(prev => ({ ...prev, [docType]: true }));

    // Simulate async network upload, malware scanning, and OCR indexing
    setTimeout(() => {
      ...
      const fileExtension = file.type.split("/")[1] || "pdf";
      const newFileObj = {
        name: file.name,
        size: file.size,
        type: file.type,
        path: `/uploads/documents/${docType.toLowerCase()}_${Date.now()}.${fileExtension}`,
        hash: mockHash,
        previewUrl,
      };
      ...
      showNotification(`${docType.replace(/_/g, " ")} processed & archived in secure storage.`, "success");
    }, 1800);
  };
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h1 className={`text-sm font-bold uppercase tracking-widest ${isDark ? "text-white" : "text-blue-950"}`}>
              Buyer Onboarding Portal
            </h1>
          </div>
          <p className="text-[10px] text-slate-500">
            Submit statutory parameters to secure platform buying and escrow billing access.
          </p>
        </div>
              ...
              <div className="flex items-center gap-3">
                {activeStep < 4 ? (
                  <button
                    ...
                    className="flex items-center gap-1.5 bg-blue-900 hover:bg-blue-800 text-white font-bold px-6 py-2.5 rounded-lg cursor-pointer transition-all shadow"
                  >
                    <span>Continue</span>
                    <ChevronRight className="h-4 w-4" />
                  </button>
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 bg-[#12589f] hover:bg-blue-800 disabled:opacity-50 text-white font-bold px-8 py-3 rounded-lg cursor-pointer transition-all shadow-xl"
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-[10px]">
                ...
                <div className={`p-4 rounded-xl border space-y-1 bg-slate-50/50 dark:bg-slate-950 border-slate-150/60 dark:border-slate-850`}>
                  ...
                  {profile.organization ? (
                    ...
                    <span className={`font-bold block ${isDark ? "text-slate-200" : "text-blue-950"}`}>Individual Buyer Record</span>
                  )}
                </div>
              </div>
            </div>
